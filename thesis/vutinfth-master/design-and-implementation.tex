...
But, the SNRM's TensorFlow graph is comprised of placeholders for input tensors,
    ...
    of opimizing the model with regard to the cost function,
...
The scope of responsibility for the SNRM class was reduced to two areas, 
    namely, to define the neural network's architecture of layers and activation functions
    in the constructor, and in the second place, to define the model's \texttt{forward} 
    function for representation inference.

% TODO add model declarations from constructor, describe it!
...
for i in range(len(self.layer_size)-1):
    conv_layer_dict['conv_' + str(i)] = 
                        nn.Conv2d(in_channels=self.layer_size[i], 
...
% embed code to show difference between model definition TF, PyTorch
% add layer etc.
...
As a result, the model's inference function was designed to have three optional formal parameters for
...
% TODO comment code of original snrm to extensive, reduction to description
